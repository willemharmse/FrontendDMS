...
import { faArrowsRotate, faCaretLeft, faCaretRight, faCirclePlus, faDownload, faEdit, faFolderOpen, faTrash } from '@fortawesome/free-solid-svg-icons';
...
import ApproveTemplateSuggestion from "./ApproveTemplateSuggestion";
...
const COLUMN_SIZE_LIMITS = {
    ...
    area: { min: 80, max: 300 },
    approvalStatus: { min: 80, max: 300 },
};

const SuggestedTaskTemplates = () => {
    ...
    useEffect(() => {
        ...
        const foundTemplate = files.find(
            (template) => String(template._id) === String(taskID)
        );

        if (!foundTemplate) {
            toast.error("Suggested template not found.", {
                autoClose: 3000,
                closeButton: false,
            });
            return;
        ...
        if (foundTemplate.approvalStatus !== null) {
            return;
        }

        setApprovingTemplate(foundTemplate);
        setIsModalOpen(true);
    }, [taskID, files]);

    const fetchFiles = async () => {
        const route = `/api/taskTemplates/suggested-templates`;
        try {
    ...
    const openApprovePopup = (template) => {
        if (template.approvalStatus !== null) {
            return
        }

        setApprovingTemplate(template);
        setIsModalOpen(true);
    };
    ...
    const handleApproveTask = async (templateValues) => {
        const storedToken = localStorage.getItem("token");
        if (!storedToken || !templateValues?._id) return;

        try {
            const response = await fetch(
                `${process.env.REACT_APP_URL}/api/taskTemplates/approve-template/${templateValues._id}`,
                {
                    method: "POST",
                    headers: {
            ...
            if (!response.ok) throw new Error(data?.error || "Failed to approve template");

            toast.success("Task template approved successfully.", {
                autoClose: 3000,
                closeButton: false,
            });
            ...
            toast.error("Failed to approve task template", {
                autoClose: 3000,
                closeButton: false,
            });
        }
    ...
    const handleDeclineTask = async () => {
        const storedToken = localStorage.getItem("token");
        if (!storedToken || !approvingTemplate?._id) return;

        try {
            const response = await fetch(
                `${process.env.REACT_APP_URL}/api/taskTemplates/decline-template/${approvingTemplate._id}`,
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${storedToken}`,
            ...
            const data = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(data?.error || "Failed to decline template");

            toast.success("Task template declined successfully.", {
                autoClose: 3000,
                closeButton: false,
            });
            ...
            toast.error("Failed to decline task template", {
                autoClose: 3000,
                closeButton: false,
            });
        }
    ...
    const resetColumnWidths = () => {
        ...
        const ww = wrapper.getBoundingClientRect().width;
        if (!ww) return;
        const defaultCols = allColumns.filter(c => !c.hidden).map(c => c.id);
    ...
    return (
        <div className="gen-file-info-container">
            {isSidebarVisible && (
                <div className="sidebar-um">
                    ...
                    <div className="sidebar-logo-dm-fi">
                        ...
                        <p className="logo-text-dm-fi">{"Suggested Templates"}</p>
                    </div>
                </div>
            )}
            ...
            <div className="main-box-gen-info">
                ...
                <div className="table-flameproof-card">
                    <div className="flameproof-table-header-label-wrapper">
                        <label className="risk-control-label">{"Suggested Task Templates"}</label>
                    ...
                    <div className="table-container-file-flameproof-all-assets">
                        ...
                        >
                            <table className={`limit-table-height-visitor ${isWide ? 'wide' : ''}`} style={{ height: "0", tableLayout: "fixed", width: tableWidth ? `${tableWidth}px` : "100%" }}>
                                <colgroup>
                                ...
                                <tbody>
                                    {processedFiles.length === 0 ? (
                                        <tr><td colSpan={visibleColumns.length} className="cent-values-gen">No Suggested Templates Found.</td></tr>
                                    ) : (
                                        processedFiles.map((file, index) => (
                                            <tr
                                                key={file._id}
                                                className="file-info-row-height gen-tr"
                                                onClick={() => openApprovePopup(file)}
            ...
            {isModalOpen && approvingTemplate && (
                <ApproveTemplateSuggestion
                    data={approvingTemplate}
                    onClose={closeApprovePopup}
                    approve={handleApproveTask}
                    decline={handleDeclineTask}
                />
            )}
            <ToastContainer />
        </div >
...
export default SuggestedTaskTemplates;