...
const ResponsibilityInfoBox = ({
    ...
    // Keep the fixed fields present in formData.taskResponsibilityDetails.
    ...
    useEffect(() => {
        setFormData(prev => {
            const existing = prev.taskResponsibilityDetails || [];
            ...
            return {
                ...prev,
                taskResponsibilityDetails: [...baseRows, ...extraRows]
            };
        });
    ...
    const handleValueChange = (field, value) => {
        setFormData(prev => ({
            ...prev,
            taskResponsibilityDetails: (prev.taskResponsibilityDetails || []).map(item =>
                item.field === field ? { ...item, value } : item
            )
        }));
    };

    const rows = formData.taskResponsibilityDetails || [];
    ...
    const handlePopupSave = (newExtraFields) => {
        setFormData(prev => {
            const existingMap = new Map((prev.taskResponsibilityDetails || []).map(item => [item.field, item.value]));
    ...
    const handleRemoveField = (field) => {
        ...
        setFormData(prev => ({
            ...prev,
            taskResponsibilityDetails: (prev.taskResponsibilityDetails || []).filter(item => item.field !== field)
        }));
    ...
    return (
        <div className="input-row">
            <div className={`input-box-ref ${error ? 'error-create' : ''}`}>
                <h3 className="font-fam-labels">Responsibility <span className="required-field">*</span></h3>
                    ...
                    <>
                                            ...
                                            <tr key={item.field}>
                                                ...
                                                <td>
                                                    <textarea
                                                        ...
                                                        onFocus={() => {
                                                            if (error) {
                                                                setErrors(prev => ({ ...prev, taskResponsibilityDetails: false }));
                                                            }
                                                        }}
                                                        readOnly={readOnly}
                                                        style={{ resize: "none" }}
                        ...
                        {!readOnly && (
                            <button
                                ...
                                title="Add Responsibility Field"
                                type="button"
                            >
            ...
            <SelectFieldsPopup
                ...
                title="Select Responsibility Fields"
...
export default ResponsibilityInfoBox;