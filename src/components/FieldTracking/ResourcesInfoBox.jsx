...
const ResourcesInfoBox = ({
    ...
    // Keep the fixed fields present in formData.taskResourceDetails.
    ...
    useEffect(() => {
        setFormData(prev => {
            const existing = prev.taskResourceDetails || [];
            ...
            return {
                ...prev,
                taskResourceDetails: [...baseRows, ...extraRows]
            };
        });
    ...
    const handleValueChange = (field, value) => {
        setFormData(prev => ({
            ...prev,
            taskResourceDetails: (prev.taskResourceDetails || []).map(item =>
                item.field === field ? { ...item, value } : item
            )
        }));
    };

    const rows = formData.taskResourceDetails || [];
    ...
    const handlePopupSave = (newExtraFields) => {
        setFormData(prev => {
            const existingMap = new Map((prev.taskResourceDetails || []).map(item => [item.field, item.value]));
    ...
    const handleRemoveField = (field) => {
        ...
        setFormData(prev => ({
            ...prev,
            taskResourceDetails: (prev.taskResourceDetails || []).filter(item => item.field !== field)
        }));
    ...
    return (
        <div className="input-row">
            <div className={`input-box-ref ${error ? 'error-create' : ''}`}>
                <h3 className="font-fam-labels">Resources <span className="required-field">*</span></h3>
                                            ...
                                            <tr key={item.field}>
                                                ...
                                                <td>
                                                    <textarea
                                                        ...
                                                        onFocus={() => {
                                                            if (error) {
                                                                setErrors(prev => ({ ...prev, taskResourceDetails: false }));
                                                            }
                                                        }}
                                                        readOnly={readOnly}
                                                        style={{ resize: "none" }}
...
export default ResourcesInfoBox;