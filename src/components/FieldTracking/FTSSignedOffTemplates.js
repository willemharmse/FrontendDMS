...
const FTSSignedOffTemplates = () => {
    ...
    const deleteFile = async () => {
        ...
        try {
            setLoading(true);
            const r = await fetch(`${process.env.REACT_APP_URL}/api/ftsGeneratedDocs/template/trashFile/${fileToDelete}`,
    ...
    // Frontend-only: a document is overdue for review if its dateReviewed has passed
    const isReviewOverdue = (file) => {
        if (!file?.dateReviewed) return false;
        const reviewDate = new Date(file.dateReviewed);
        if (isNaN(reviewDate.getTime())) return false;

        const today = new Date();
        // Compare by day only, so a review due "today" isn't flagged as overdue
    ...
    const downloadFile = async (fileId, fileName) => {
        try {
            setLoading(true);
            const r = await fetch(`${process.env.REACT_APP_URL}/api/file/generatedTemplates/download/${fileId}`,
    ...
    const getFilterValuesForCell = (row, colId, index) => {
        ...
        if (colId === "status") return [getStatus(row)];
    ...
    const processedFiles = useMemo(() => {
        ...
        const effectiveColId = colId ?? "firstPublishedDate";

        const dir = direction === "desc" ? -1 : 1;
        if (effectiveColId !== "nr") {
            const normalize = (v) => { const s = v == null ? "" : String(v).trim(); return s === "" ? "(Blanks)" : s; };
            current.sort((a, b) => {
                let valA, valB;
                switch (effectiveColId) { case "name": valA = a.formData.title; valB = b.formData.title; break; case "version": valA = a.formData.version; valB = b.formData.version; break; case "status": valA = getStatus(a); valB = getStatus(b); break; case "firstPublishedBy": valA = a.publisher?.username; valB = b.publisher?.username; break; case "firstPublishedDate": valA = a.datePublished; valB = b.datePublished; break; case "lastReviewedBy": valA = a.reviewer?.username; valB = b.reviewer?.username; break; case "lastReviewDate": valA = a.dateReviewed; valB = b.dateReviewed; break; default: valA = a[colId]; valB = b[colId]; }
                if (["firstPublishedDate", "lastReviewDate"].includes(effectiveColId)) { return (new Date(valA) - new Date(valB)) * dir; }
                if (effectiveColId === "version") return (Number(valA) - Number(valB)) * dir;
    ...
    const allColumns = [
        ...
        {
            id: "name", title: "Template Name", thClass: "gen-th ibraGenFN", tdClass: "gen-point", onCellClick: (f) => setHoveredFileId(hoveredFileId === f._id ? null : f._id),
            td: (f) => ( //*NB
        ...
        { id: "status", title: "Status", thClass: "gen-th ibraGenStatus", tdClass: "cent-values-gen gen-point", td: (f) => getStatus(f) },
        { id: "firstPublishedBy", title: "Owner", thClass: "gen-th ibraGenPB", tdClass: "cent-values-gen gen-point", td: (f) => f.publisher.username },
        { id: "firstPublishedDate", title: "Published Date", thClass: "gen-th ibraGenPD", tdClass: "cent-values-gen gen-point", td: (f) => formatDate(f.datePublished) },
        { id: "lastReviewDate", title: "Review Date", thClass: "gen-th ibraGenRD", tdClass: "cent-values-gen gen-point", td: (f) => f.dateReviewed ? formatDate(f.dateReviewed) : "N/A" },
        {
            id: "action",
            title: "Action",
    ...
    const hasActiveFilters = useMemo(() => {
        ...
        const hasSort = sortConfig.colId !== null || sortConfig.direction !== "desc";
        console.log(hasSort)
    ...
    const handleClearFilters = () => {
        ...
        setSortConfig({ colId: null, direction: "desc" });
    ...
    return (
        <div className="gen-file-info-container">
                ...
                <div className="sidebar-um">
                    ...
                    <div className="button-container-create">
                        <button className="but-um"
                            onClick={() => navigate('/FrontendDMS/ftsDeletedTemplates')}>
            ...
            {!isSidebarVisible && (<div className="sidebar-hidden"><div className="sidebar-toggle-icon" onClick={() => setIsSidebarVisible(true)}><FontAwesomeIcon icon={faCaretRight} /></div></div>)}
            ...
            <div className="main-box-gen-info">
                <div className="top-section-um">
                    ...
                    <div className="um-input-container">
                        <input className="search-input-um"
                            type="text" placeholder="Search"
                            value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} autoComplete="off"
                        />
                        {searchQuery ? <i><FontAwesomeIcon icon={faX} onClick={clearSearch} className="icon-um-search" /></i> : <i><FontAwesomeIcon icon={faSearch} className="icon-um-search" /></i>}
                    </div>

                    <div className={`info-box-fih`}>
                        Number of Templates: {processedFiles.length}
                    </div>
                    ...
                    <div className="table-container-file-flameproof-all-assets">
                        <div className={`limit-table-height-visitor-wrap ${isDraggingX ? 'dragging' : ''} ${isWide ? 'wide' : ''}`} ref={scrollerRef} onPointerDown={onPointerDownX} onPointerMove={onPointerMoveX} onPointerUp={endDragX} onPointerLeave={endDragX} onDragStart={(e) => e.preventDefault()} style={{ maxHeight: "calc(100% - 0px)", height: "100%" }}>
                            <table className={`limit-table-height-visitor ${isWide ? 'wide' : ''}`} style={{ height: "0" }}>
                                ...
                                <tbody>{processedFiles.length === 0 ? <tr><td colSpan={visibleColumns.length} className="cent-values-gen">No documents found.</td></tr> : processedFiles.map((file, index) => (<tr key={file._id} className="file-info-row-height gen-tr">{visibleColumns.map(col => (<td key={`${file._id}-${col.id}`} className={`${col.tdClass} ${col.id === "status" ? getStatusClass(file) : ""}`} onClick={col.onCellClick ? () => col.onCellClick(file) : undefined}>{col.td(file, index)}</td>))}</tr>))}</tbody>
                            </table>
                        </div>
                    </div>
                </div>
...
export default FTSSignedOffTemplates;