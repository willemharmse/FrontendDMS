...
const SafetyInfoBox = ({
    ...
    // Keep the fixed fields present in formData.taskSafetyDetails.
    ...
    useEffect(() => {
        setFormData(prev => {
            const existing = prev.taskSafetyDetails || [];
            ...
            return {
                ...prev,
                taskSafetyDetails: [...baseRows, ...extraRows]
            };
        });
    ...
    const handleValueChange = (field, value) => {
        setFormData(prev => ({
            ...prev,
            taskSafetyDetails: (prev.taskSafetyDetails || []).map(item =>
                item.field === field ? { ...item, value } : item
            )
        }));
    };

    const rows = formData.taskSafetyDetails || [];
    ...
    const handlePopupSave = (newExtraFields) => {
        setFormData(prev => {
            const existingMap = new Map((prev.taskSafetyDetails || []).map(item => [item.field, item.value]));
    ...
    const handleRemoveField = (field) => {
        ...
        setFormData(prev => ({
            ...prev,
            taskSafetyDetails: (prev.taskSafetyDetails || []).filter(item => item.field !== field)
        }));
    ...
    return (
        <div className="input-row">
            <div className={`input-box-ref ${error ? 'error-create' : ''}`}>
                <h3 className="font-fam-labels">Safety and Risk <span className="required-field">*</span></h3>
                                            ...
                                            <tr key={item.field}>
                                                ...
                                                <td>
                                                    <textarea
                                                        ...
                                                        onFocus={() => {
                                                            if (error) {
                                                                setErrors(prev => ({ ...prev, taskSafetyDetails: false }));
                                                            }
                                                        }}
                                                        readOnly={readOnly}
                                                        style={{ resize: "none" }}
