...
const CloseOutInfoBox = ({
    ...
    // Keep the fixed fields present in formData.taskCloseOutDetails.
    ...
    useEffect(() => {
        setFormData(prev => {
            const existing = prev.taskCloseOutDetails || [];
            ...
            return {
                ...prev,
                taskCloseOutDetails: [...baseRows, ...extraRows]
            };
        });
    ...
    const handleValueChange = (field, value) => {
        setFormData(prev => ({
            ...prev,
            taskCloseOutDetails: (prev.taskCloseOutDetails || []).map(item =>
                item.field === field ? { ...item, value } : item
            )
        }));
    };

    const rows = formData.taskCloseOutDetails || [];
    ...
    const handlePopupSave = (newExtraFields) => {
        setFormData(prev => {
            const existingMap = new Map((prev.taskCloseOutDetails || []).map(item => [item.field, item.value]));
    ...
    const handleRemoveField = (field) => {
        ...
        setFormData(prev => ({
            ...prev,
            taskCloseOutDetails: (prev.taskCloseOutDetails || []).filter(item => item.field !== field)
        }));
    ...
    return (
        <div className="input-row">
            <div className={`input-box-ref ${error ? 'error-create' : ''}`}>
                <h3 className="font-fam-labels">Close Out Information <span className="required-field">*</span></h3>
                                            ...
                                            <tr key={item.field}>
                                                ...
                                                <td>
                                                    <textarea
                                                        ...
                                                        onFocus={() => {
                                                            if (error) {
                                                                setErrors(prev => ({ ...prev, taskCloseOutDetails: false }));
                                                            }
                                                        }}
                                                        readOnly={readOnly}
                                                        style={{ resize: "none" }}
...
export default CloseOutInfoBox;