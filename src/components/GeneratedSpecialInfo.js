...
const GeneratedSpecialInfo = () => {
    ...
    const deleteFile = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/special/trashFile/${fileToDelete}`, {
                headers: {
            ...
            <div className="main-box-gen-info">
                ...
                <div className="table-flameproof-card">
                    <div className="flameproof-table-header-label-wrapper">
                        <label className="risk-control-label">{"Ready for Approval Special Instructions"}</label>
                    </div>
                            ...
                            <tbody>
                                {filteredFiles.map((file, index) => (
                                    <tr key={file._id} className={`file-info-row-height gen-tr`}>
                                        ...
                                        <td className={`${getStatusClass(file.documentStatus)} cent-values-gen  gen-point`}>{getStatus(file.documentStatus)}</td>
