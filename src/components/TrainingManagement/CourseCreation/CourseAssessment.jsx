...
const CourseAssessment = ({ formData, setFormData }) => {
    const addQuestion = () => {
        setFormData(prev => ({
            ...prev,
            assessment: [
                ...prev.assessment,
                { id: crypto.randomUUID(), question: "", answer: "", options: ["", "", ""] }
            ]
        }));
    };
    ...
    const removeQuestion = (qid) => {
        setFormData(prev => ({
            ...prev,
            assessment: prev.assessment.filter(q => q.id !== qid)
        }));
    };
    ...
    const removeOption = (qid, optIndex) => {
        setFormData(prev => ({
            ...
            assessment: prev.assessment.map(q => {
                ...
                // if the removed option was marked correct, clear answer
                                ...
                                {(question.options || []).map((opt, optIndex) => (
                                    <div className="course-ass-file-option-row" key={`${question.id}-${optIndex}`}>
                                        <div className="course-ass-file-radio-wrap">
                                            <input
                                                type="radio"
                                                name={`q-${question.id}`}          /* one correct option per question */
