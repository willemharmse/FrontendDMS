...
import { faArrowLeft, faCaretLeft, faCaretRight, faFilter } from "@fortawesome/free-solid-svg-icons";
import TopBarDD from "../../Notifications/TopBarDD";
import { ToastContainer } from "react-toastify";
import OTChatPanelLecturer from "./OTChatPanelLecturer";
import OTChatPanelLecturerClaimed from "./OTChatPanelLecturerClaimed";
...
const OTClaimedChats = () => {
        ...
        const loadUnclaimedChats = async () => {
            try {
                ...
                const res = await fetch(
                    `${process.env.REACT_APP_URL}/api/chatBoxOnlineTraining/claimed`,
                    {
                        headers: {
    ...
    const getFilterValuesForCell = (row, colId) => {
        let val;
        if (colId === "courseName") val = row.courseName;
        if (colId === "studentName") val = row.studentName;
        if (colId === "unreadMessages") val = row.unreadMessages;
        if (colId === "lastMessageTimeSent") val = formatDate(row.lastMessageTimeSent); // Filter by the visible string
    ...
    const filteredChats = useMemo(() => {
        ...
        current.sort((a, b) => {
            ...
            if (colId === "lastMessageTimeSent") {
                const ad = a.lastMessageTimeSent ? new Date(a.lastMessageTimeSent).getTime() : 0;
                const bd = b.lastMessageTimeSent ? new Date(b.lastMessageTimeSent).getTime() : 0;
                return (ad - bd) * dir;
            }
        ...
        <div className="dc-version-history-file-info-container">
            ...
            <div className="main-box-dc-version-history-file">
                ...
                <div className="table-flameproof-card">
                    <div className="flameproof-table-header-label-wrapper">
                        <label className="risk-control-label">Claimed Chats</label>
                        <FontAwesomeIcon
                            icon={faFilter}
                            ...
                            <thead className="dc-version-history-file-info-head">
                                <tr>
                                    ...
                                    <th
                                        ...
                                        onClick={(e) => openExcelFilterPopup("courseName", e)}
                                    >
                                        Course Name
                                        {(filters["courseName"] || sortConfig.colId === "courseName") && (
                                            <FontAwesomeIcon
                                                icon={faFilter}
                                                className="active-filter-icon"
                                                style={{ marginLeft: "10px", color: filters["courseName"] ? "#2196F3" : "inherit" }}
                                            />
                                        )}
                                    </th>
                                    <th
                                    ...
                                    <th
                                        ...
                                        onClick={(e) => openExcelFilterPopup("unreadMessages", e)}
                                    >
                                        Unread Messages
                                        {(filters["unreadMessages"] || sortConfig.colId === "unreadMessages") && (
                                            <FontAwesomeIcon
                                                icon={faFilter}
                                                className="active-filter-icon"
                                                style={{ marginLeft: "10px", color: filters["unreadMessages"] ? "#2196F3" : "inherit" }}
                                            />
                                        )}
                                    </th>
                                    <th
                                        style={{ width: "20%", textAlign: "center", cursor: "pointer" }}
                                        onClick={(e) => openExcelFilterPopup("lastMessageTimeSent", e)}
                                    >
                                        Last Message Time Sent
                                        {(filters["lastMessageTimeSent"] || sortConfig.colId === "lastMessageTimeSent") && (
                                            <FontAwesomeIcon
                                                icon={faFilter}
                                                className="active-filter-icon"
                                                style={{ marginLeft: "10px", color: filters["lastMessageTimeSent"] ? "#2196F3" : "inherit" }}
                                            />
                                        )}
                                    </th>
                                </tr>
                            ...
                            <tbody>
                                {filteredChats.map((chat, index) => (
                                    ...
                                    >
                                        ...
                                        <td style={{ textAlign: "center" }}>{chat.courseName}</td>
                                        <td style={{ textAlign: "center" }}>{chat.studentName}</td>
                                        <td style={{ textAlign: "center" }}>{chat.unreadMessages}</td>
                                        <td style={{ textAlign: "center" }}>{formatDate(chat.lastMessageTimeSent)}</td>
                                    </tr>
                                ))}
            ...
            {chatOpen &&
                ...
                localStorage.getItem("token") && (
                    <OTChatPanelLecturerClaimed
                        open={chatOpen}
                        onClose={() => { setChatOpen(false); setSelectedChat(null); }}
