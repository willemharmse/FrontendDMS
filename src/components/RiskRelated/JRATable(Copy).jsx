        ...
        for (const [colId, text] of Object.entries(filters)) {
            ...
            if (colId === "nr") {
                ...
                survivingBodies = survivingBodies.filter(body => {
                    const values = extractBodyValues(body, colId);
                    // if there are no values, or all of them are just empty strings,
                    // treat it as “blank” and keep the row
                    if (
                        values.length === 0 ||
                    ...
                    // otherwise only keep if at least one value matches the filter
                    return values.some(v =>
                        v.toLowerCase().includes(lcText)
                    );
                })
                                                        ...
                                                        return (
                                                            <td key={colIdx} className={`hazard-cell`}>
                                                                {body.hazards.map((hObj, hIdx) => {
                                                                    const isFirst = (bodyIdx === 0 && hObj.hazard === "Work Execution");
                                                                    if (isFirst) {
                                                                        return (
                                                        ...
                                                        return (
                                                            <td key={colIdx} className={`${cls}`} >
                                                                {body.UE.map((uObj, uIdx) => {
                                                                    const isFirst = (bodyIdx === 0 && uObj.ue === "Non-adherence to task step requirements / specifications");
