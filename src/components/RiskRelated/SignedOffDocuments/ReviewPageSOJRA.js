...
import { faFloppyDisk, faCheckCircle, faSpinner, faRotateLeft, faCalendarDays, faInfoCircle, faMagicWandSparkles, faArrowLeft, faBell, faCircleUser, faChevronLeft, faChevronRight, faCaretLeft, faCaretRight, faRotateRight, faSave, faPen, faUpload } from '@fortawesome/free-solid-svg-icons';
...
import MaterialsTableRisk from "../RiskComponents/MaterialsTableRisk";
import HandToolsTableRisk from "../RiskComponents/HandToolsTableRisk";
import PPETableRisk from "../RiskComponents/PPETableRisk";
import IntroTaskInfo from "../IntroTaskInfo";
import MobileMachineTableRisk from "../RiskComponents/MobileMachineTableRisk";
import JRATable from "../JRATable";
import OtherTeam from "../OtherTeam";
import EquipmentTableRisk from "../RiskComponents/EquipmentTableRisk";
import ApproversPopup from "../../VisitorsInduction/InductionCreation/ApproversPopup";

const ReviewPageSOJRA = () => {
    ...
    const saveData = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffJRA/save/${fileID}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    const getNewAzureFileName = async () => {
        try {
            ...
            const response = await fetch(
                `${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffJRA/getFile/${fileID}`,
                {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const handlePublishApprovalFlow = async (approversValue) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_URL}/api/riskApprovals/start-approval-jra-signedOff`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
    ...
    const approveDraft = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_URL}/api/riskApprovals/approve-signedOff-jra`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
    ...
    const sendUpdatedFormData = async (formDataToStore, documentName) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_URL}/api/t/publish-signedOff-JRA`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
...
export default ReviewPageSOJRA;