...
const DeletedDocumentsSOBLRA = () => {
    ...
    const deleteFile = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffBLRA/deleteFile/${fileToDelete}`, {
    ...
    const restoreFile = async (fileId) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffBLRA/restoreFile/${fileId}`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchFiles = async () => {
        const route = `/api/fileGenDocs/signedOffBLRA/trash/getFiles`;
        try {
...
export default DeletedDocumentsSOBLRA;