...
const DeletedRiskDocumentsIBRA = () => {
    ...
    const deleteFile = async () => { if (!fileToDelete) return; try { setLoading(true); const r = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/ibra/deleteFile/${fileToDelete}`, { headers: { Authorization: `Bearer ${token}` }, method: 'DELETE', }); if (!r.ok) throw new Error('Failed'); setFileToDelete(""); setSelectedFileName(""); setIsModalOpen(false); fetchFiles(); } catch (e) { console.error(e); } finally { setLoading(false); } };
    const restoreFile = async (fileId) => { try { const r = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/ibra/restoreFile/${fileId}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` } }); if (!r.ok) throw new Error('Failed'); fetchFiles(); } catch (e) { alert('Error restoring'); } };
    ...
    const fetchFiles = async () => { try { const r = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/ibra/trash/getFiles`, { headers: { 'Authorization': `Bearer ${token}` } }); if (!r.ok) throw new Error('Failed'); const d = await r.json(); setFiles(d.files); } catch (e) { setError(e.message); } };
...
export default DeletedRiskDocumentsIBRA;