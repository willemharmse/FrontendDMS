...
import { faPlusCircle, faSpinner, faTrash, faTrashCan, faPlus, faInfoCircle } from '@fortawesome/free-solid-svg-icons';

const DocumentSignaturesRiskTable = ({ rows, handleRowChange, addRow, removeRow, error, updateRows }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        ["Facilitator A", "Facilitator B", "Facilitator C"].forEach((name) => {
          positionMap[name] = "Facilitator Position"; // or different positions if you want
        });

        setNameLists({
          Approver: approvers.map((a) => a.name),
          Owner: authors.map((a) => a.name),
          Reviewer: reviewers.map((a) => a.name),
          Facilitator: ["Facilitator A", "Facilitator B", "Facilitator C"],
        });
  ...
  return (
    <div className="input-row">
      <div className={`input-box-sig-risk ${error ? "error-sign" : ""}`}>
        <button
          className="top-left-button-signature"
          title="Information"
        >
