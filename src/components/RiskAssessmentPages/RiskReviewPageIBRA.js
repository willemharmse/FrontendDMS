...
const RiskReviewPageIBRA = () => {
    ...
    const removeFromApprovalProcess = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_URL}/api/riskApprovals/remove-from-approval-ibra-published`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("token")}`
            ...
            toast.success(`IBRA Removed From The Approval Process.`, {
                closeButton: true,
                autoClose: 800, // 1.5 seconds
                style: {
                    textAlign: 'center'
