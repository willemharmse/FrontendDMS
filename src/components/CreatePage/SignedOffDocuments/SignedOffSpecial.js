...
const SignedOffSpecial = () => {
    ...
    const reviewDocument = async (fileID) => {
        let route = `${process.env.REACT_APP_URL}/api/docCreate/reviewSOSpecialIns/${fileID}`
    ...
    const fetchFiles = async () => { try { const r = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffSpecial/${userID}`); if (!r.ok) throw new Error('Failed'); const d = await r.json(); setFiles(d.files); console.log(d.files) } catch (e) { setError(e.message); } };
    ...
    const allColumns = [
        ...
        { id: "name", title: "Document Name", thClass: "gen-th ibraGenFN", tdClass: "gen-point", onCellClick: (f) => setHoveredFileId(hoveredFileId === f._id ? null : f._id), td: (f) => (<div className="popup-anchor"><span>{removeFileExtension(f.formData.title)}</span>{(hoveredFileId === f._id) && (<PopupMenuSignedOffFiles file={f} typeDoc={"special"} risk={false} isOpen={true} openDownloadModal={downloadFile} setHoveredFileId={setHoveredFileId} id={f._id} review={reviewDocument} />)}</div>) },
                ...
                <div className="sidebar-um">
                    ...
                    <div className="button-container-create">
                        <button className="but-um"
                            onClick={() => navigate('/FrontendDMS/deletedSignedOffSpecial')}>
                    ...
                    <div className="sidebar-logo-dm-fi">
                        <img src={`${process.env.PUBLIC_URL}/specialInstInverted.svg`} className="icon-risk-rm" /><p className="logo-text-dm-fi">{"Signed Off  Special Instructions"}</p>
                    </div>
                </div>)
            }
            ...
            <div className="main-box-gen-info">
                <div className="top-section-um">
                    ...
                    <div className={`info-box-fih`}>
                        Number of Documents: {processedFiles.length}
                    </div>
                ...
                <div className="table-flameproof-card">
                    <div className="flameproof-table-header-label-wrapper">
                        <label className="risk-control-label">{"Signed Off Special Instructions"}</label>
...
export default SignedOffSpecial;