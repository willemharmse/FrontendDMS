...
const DeletedDocumentsStandard = () => {
    ...
    const deleteFile = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/standard/deleteFile/${fileToDelete}`, {
    ...
    const restoreFile = async (fileId) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_URL}/api/fileGenDocs/standard/restoreFile/${fileId}`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchFiles = async () => {
        const route = `/api/fileGenDocs/standard/trash/getFiles`;
        try {
    ...
    const toggleSort = (colId, direction) => {
        setSortConfig(prev => {
            if (prev?.colId === colId && prev?.direction === direction) {
                return { colId: null, direction: null }; // Reset to default
            }
    ...
    useEffect(() => {
        ...
        if (newTop !== excelFilter.pos.top || newLeft !== excelFilter.pos.left) {
            // We can manipulate the DOM directly for perf or set state
...
export default DeletedDocumentsStandard;