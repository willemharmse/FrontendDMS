...
// pattern GeneratedFileInfo uses (allColumns: id/title/td) instead of the
// switch-based getRawValue approach, so the columns (and how each one's
// value/title is derived) match GeneratedFileInfo exactly.
// The "Action" column is defined but hidden for now — flip HIDDEN_COLUMNS
// (or drop "action" from it) to bring it back later.
const HIDDEN_COLUMNS = ["action"];

const CreateRevisionPage = () => {
    ...
    // Same type -> config pattern as ApprovalsPage, just pointed at the
    // "Under Revision" workflow/routes instead of review & approval ones.
    ...
    // --- Column config map (mirrors GeneratedFileInfo's allColumns) ---
                        ...
                        <table className="gen-table">
                            ...
                            <tbody>
                                ...
                                {!isLoading && drafts.length === 0 && showNoDrafts && (
                                    <tr>
                                        <td colSpan={visibleColumns.length} className="cent">
                                            No Documents Currently Under Revision
                                        </td>
                                    </tr>
                                )}
                            </tbody>
